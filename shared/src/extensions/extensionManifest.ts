 ...
 * JSON Schema. For simplicity, only necessary fields are validated and included here. (For example, the
 * "description" field is not checked, because it is not currently used by any callers. When shown in the UI, it's
 * queried from the GraphQL API.)
 */
export interface ExtensionManifest extends Pick<ExtensionManifestSchema, 'url' | 'activationEvents' | 'contributes'> {}
